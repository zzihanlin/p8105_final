...
renderPlotly( 
  final_df|> 
    ...
    ggplot( aes(x = year, y = prevalence_percent, group = state, color = state)) +
    geom_line() +
    geom_point() +
    labs(
      title = paste("Asthma Prevalence Over Time in", input$state_choice),
      x = "Year",
      y = "Asthma Prevalence (%)"
    ) +
...
renderPlotly(
  final_df|>
    ...
    year == input[["year_choice"]]
     ...
     mutate(state = fct_reorder(state, year))|>
    plot_ly(x = ~state, y = ~prevalance_percent, type = "box" )
  )
```
